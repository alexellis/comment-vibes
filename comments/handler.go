...
func Handle(w http.ResponseWriter, r *http.Request) {
	...
	rows, getErr := db.Query(`select * from get_emojis();`)

	if getErr != nil {
		log.Printf("get error: %s", getErr.Error())
		http.Error(w, errors.Wrap(getErr, "unable to get from get_emojis").Error(),
	...
	for rows.Next() {
		result := Result{}
		scanErr := rows.Scan(&result.Emoji, &result.Total)
		if scanErr != nil {
